...
@if($type->author && !is_array($type->author))
    @include('moonshine-seo::open-graph.types.profile', ['type' => $type->author, 'alias' => 'article:author'])
@endif
@if(is_array($type->author) && count($type->author))
@foreach($type->author as $profile)
    @include('moonshine-seo::open-graph.types.profile', ['type' => $profile, 'alias' => 'article:author'])
@endforeach
@endif
@if($type->section)
