@if($type->author && !is_array($type->author))
    @include('moonshine-seo::open-graph.types.profile', ['type' => $type->author, 'alias' => 'book:author'])
@endif
@if(is_array($type->author) && count($type->author))
@foreach($type->author as $profile)
    @include('moonshine-seo::open-graph.types.profile', ['type' => $profile, 'alias' => 'book:author'])
@endforeach
@endif
@if($type->isbn)
